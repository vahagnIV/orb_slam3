...
namespace test {
class MonocularFrameOldTests;
}

class MonocularFrameOld : public FrameBase {
  friend class MonocularFrameOldTests;
 public:
  MonocularFrameOld(const TImageGray8U & image,
                 TimePoint timestamp,
 ...
 protected:
  bool ComputeMatchesForLinking(MonocularFrameOld * from_frame, std::unordered_map<size_t, size_t> & out_matches);
  ...
  void ComputeBow();
  bool BaselineIsNotEnough(const MonocularFrameOld * other) const;
  void CreateNewMpPoints(MonocularFrameOld * frame,
                         const std::unordered_map<std::size_t, std::size_t> & matches,
                         std::unordered_set<map::MapPoint *> & out_map_points);
  static optimization::edges::SE3ProjectXYZPose * CreateEdge(map::MapPoint * map_point, MonocularFrameOld * frame);
  void ComputeMatches(MonocularFrameOld * reference_kf,
