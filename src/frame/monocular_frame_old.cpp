...
MonocularFrameOld::MonocularFrameOld(const TImageGray8U & image, TimePoint timestamp,
...
bool MonocularFrameOld::IsValid() const {
  return features_.descriptors.size() > constants::MINIMAL_FEATURE_COUNT_PER_FRAME_MONOCULAR;
}

bool MonocularFrameOld::ComputeMatchesForLinking(MonocularFrameOld * from_frame,
...
void MonocularFrameOld::InitializeMapPointsFromMatches(const unordered_map<std::size_t, std::size_t> & matches,
                                                    const std::unordered_map<size_t, TPoint3D> & points,
                                                    MonocularFrameOld * from_frame,
...
bool MonocularFrameOld::Link(FrameBase * other) {
  ...
  MonocularFrameOld * from_frame = dynamic_cast<MonocularFrameOld *>(other);
  ...
  for (auto map_point: map_points) {
    ...
    for (auto observation: map_point->Observations()) {
      if (all_edges_pass_threshold) {
        dynamic_cast<MonocularFrameOld *>(observation.first)->recent_observations_->insert(observation.second);
...
//    }
    ...
    /*for (auto mp: map_points_) {
      std::map<MonocularFrameOld *, cv::Mat>
      ...
      for (auto obs: mp.second->Observations()) {
        ...
        auto frame = dynamic_cast<MonocularFrameOld *> (observation->GetFrame());
...
TVector3D MonocularFrameOld::GetNormal(const TPoint3D & point) const {
  TPoint3D normal = inverse_pose_.T - point;
  normal.normalize();
  return normal;
}

bool MonocularFrameOld::TrackWithReferenceKeyFrame(FrameBase * reference_keyframe) {
  ...
  auto reference_kf = dynamic_cast<MonocularFrameOld *>(reference_keyframe);
...
void MonocularFrameOld::ComputeBow() {
  features_.ComputeBow();
}

void MonocularFrameOld::OptimizePose(std::unordered_set<std::size_t> & out_inliers) {
/*
...
precision_t MonocularFrameOld::ComputeMedianDepth() const {
...
bool MonocularFrameOld::BaselineIsNotEnough(const MonocularFrameOld * other) const {
...
void MonocularFrameOld::ComputeMatches(MonocularFrameOld * reference_kf,
...
/*
void MonocularFrameOld::FindNewMapPointMatches(MonocularFrameOld * keyframe,
                                            std::unordered_map<std::size_t, std::size_t> & out_matches) {
...
*/
void MonocularFrameOld::CreateNewMpPoints(MonocularFrameOld * keyframe,
...
void MonocularFrameOld::SearchLocalPoints(unordered_set<map::MapPoint *> & all_candidate_map_points) {
...
/*
optimization::edges::SE3ProjectXYZPose * MonocularFrameOld::CreateEdge(map::MapPoint * map_point, MonocularFrameOld * frame) {
...
bool MonocularFrameOld::FindNewMapPoints() {

  if (neighbour_keyframes == fixed_frames) {
    for (auto map_point: existing_local_map_points) {
      for (auto obs : map_point->Observations()) {
        if (dynamic_cast<MonocularFrameOld *>(obs.first)->map_points_.find(obs.second)
            == dynamic_cast<MonocularFrameOld *>(obs.first)->map_points_.end())
          dynamic_cast<MonocularFrameOld *>(obs.first)->AddMapPoint(map_point, obs.second);
      }
      map_point->Refresh(feature_extractor_);
    }
  ...
  std::unordered_map<map::MapPoint *, std::unordered_set<MonocularFrameOld *>> inliers;
  ...
  for (auto map_point: existing_local_map_points) {
    ...
    for (auto obs: observations) {
      auto frame = dynamic_cast<MonocularFrameOld *>(obs.first);
  ...
  for (auto map_point: existing_local_map_points) {
    if (map_point->Observations().size() <= 1) {
      for (auto frame: map_point->Observations()) {
        dynamic_cast<MonocularFrameOld *>(frame.first)->EraseMapPoint(frame.second);
      }
      if (map_point->Id() >= min_new_map_point_id)
        ++new_deleted;
      else
...
*/
bool MonocularFrameOld::IsVisible(map::MapPoint * map_point,
...
void MonocularFrameOld::FilterVisibleMapPoints(const std::unordered_set<map::MapPoint *> map_points,
...
void MonocularFrameOld::FindCandidateMapPointMatchesByProjection(const list<VisibleMapPoint> & filtered_map_points,
...
bool MonocularFrameOld::FindNewMapPointsAndAdjustPosition(const std::unordered_set<map::MapPoint *> & all_candidate_map_points) {
...
#ifndef NDEBUG
  {
//    std::map<MonocularFrameOld *, std::unordered_map<std::size_t, size_t>> match_map;
//    for (auto match:matches) {
//      map::MapPoint * mp = match.first;
//      size_t current_feature_id = match.second;
//      for (auto obs: mp->Observations()) {
//        match_map[dynamic_cast<MonocularFrameOld *>(obs.first)][current_feature_id] = obs.second;
//      }
//    }
...
void MonocularFrameOld::AddMapPoint(Observation * observation) {
...
MonocularFrameOld::~MonocularFrameOld() {
//  for (auto & mp_id: map_points_) {
...
void MonocularFrameOld::CreateNewMapPoints(FrameBase * other) {
  ComputeBow();
  ...
  auto other_frame = dynamic_cast<MonocularFrameOld *>(other);
  if (other_frame == this)
    return;
...
void MonocularFrameOld::ListMapPoints(unordered_set<map::MapPoint *> & out_map_points) const {
  for (auto mp: map_points_)
    out_map_points.insert(mp.second);
}

void MonocularFrameOld::EraseMapPoint(map::MapPoint * map_point) {
