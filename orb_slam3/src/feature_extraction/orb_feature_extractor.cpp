...
ORBFeatureExtractor::ORBFeatureExtractor(unsigned image_width,
  ...
  precision_t number_of_desired_features_per_scale = features * (1 - factor) / (1 - pow(factor, levels));

  size_t total_features = 0;
  for (size_t level = 0; level < scale_factors_.size() - 1; level++) {
    features_per_level_[level] = std::round(number_of_desired_features_per_scale);
...
const int
    ORBFeatureExtractor::umax_[HALF_PATCH_SIZE + 1] = {15, 15, 15, 15, 14, 14, 14, 13, 13, 12, 11, 10, 9, 8, 6, 3};
